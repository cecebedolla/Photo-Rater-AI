...
function renderGallery() {
  if (!state.photos.length) {
    ...
    elements.gallery.innerHTML = '<span class="empty-icon">✧</span><strong>Your ranked edit starts here.</strong><small>Upload or drag in a full shoot to generate previews, strict category scores, and preference signals.</small>';
    return;
  }
  elements.gallery.className = 'photo-grid';
  elements.gallery.innerHTML = state.photos.map((photo) => `<button class="photo-tile ${photo.id === state.activeId ? 'active' : ''}" data-photo-id="${photo.id}"><img src="${photo.previewUrl}" alt="${escapeHtml(photo.name)}" loading="lazy"><span class="score-badge ${photo.overall ? 'scored' : ''}">${photo.overall ? `${photo.overall}/10` : 'Queued'}</span><span class="tile-name">${escapeHtml(photo.name)}</span></button>`).join('');
...
function renderDetail() {
  ...
  if (!photo) {
    elements.detailPanel.innerHTML = '<div class="empty-state"><span class="empty-icon">◎</span><strong>No photo selected yet.</strong><small>Choose an image from the gallery to see its AI-style scorecard.</small></div>';
    return;
  }
  elements.detailPanel.innerHTML = `<div class="detail-image-wrap"><img class="detail-image" src="${photo.previewUrl}" alt="${escapeHtml(photo.name)}"><span class="detail-score">${photo.overall ? `${photo.overall}/10` : 'Awaiting AI'}</span></div><div class="detail-header"><div><p class="eyebrow small">Selected photo</p><h2>${escapeHtml(photo.name)}</h2><p>${formatBytes(photo.size)}</p></div></div><div class="score-list">${ratingCategories.map((category) => `<div class="score-row"><span>${category.label}</span><meter min="1" max="10" value="${photo.scores?.[category.key] ?? 1}"></meter><b>${photo.scores?.[category.key] ?? '—'}</b></div>`).join('')}</div><div class="preference-actions"><button class="${photo.preference === 'liked' ? 'selected' : ''}" data-pref="liked">♡ Like</button><button class="${photo.preference === 'disliked' ? 'selected' : ''}" data-pref="disliked">Pass</button><button data-pref="neutral">Neutral</button></div><ul class="notes">${(photo.aiNotes ?? ['Run AI scoring to generate strict rubric feedback.']).map((note) => `<li>${escapeHtml(note)}</li>`).join('')}</ul>`;
